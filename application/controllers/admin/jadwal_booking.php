...
class jadwal_booking extends CI_Controller {
	...
	function __construct() {
        ...
        $this->load->model('admin/m_jadwal_booking');
 	}

 	function index(){
 		$data['history'] = $this->m_jadwal_booking->show_jadwal();
		$this->load->view('admin/v_jadwal_booking',$data);
    }
	...
	function reschedule(){
		...
		if ($this->form_validation->run() == TRUE) 
		{
			...
			if ($tgl_meeting < $datenow){
          		redirect(base_url() . 'admin/jadwal_booking/reschedule/' . $id_booking . '/' . 2);
        	}else if ($start_time > $end_time){
          		redirect(base_url() . 'admin/jadwal_booking/reschedule/' . $id_booking .'/'. 3);
			...
			$this->load->view('admin/v_form_reschedule',$data);
		}else{
	...
	function reschedule_exc(){
		...
		if ($this->form_validation->run() == TRUE) 
		{
		    ...
		    if( $start_time > $row_c->jam_mulai && $start_time < $row_d->jam_akhir || $end_time > $row_c->jam_mulai && $end_time < $row_d->jam_akhir){
		    	redirect(base_url() . 'admin/jadwal_booking/reschedule/'. $id_booking .'/1');
