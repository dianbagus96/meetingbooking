...
class book_room extends CI_Controller {
	...
	function __construct() {
      ...
      $this->load->model('admin/m_book_room');

 	}

  function index(){
      $data['alert'] = "";
      $this->load->view('admin/v_book_room',$data);  
  }
    ...
    if ($this->form_validation->run() == TRUE) 
    {
        if ($tgl_meeting < $tgl_psn){
          redirect(base_url() . 'admin/book_room/search_room/' . 2);
        }else if ($start_time > $end_time){
          redirect(base_url() . 'admin/book_room/search_room/' . 3);
        ...
        $this->load->view('admin/v_form_book_room',$data);
    }else{
        $flag = 0;
  ...
  function book_room_exc(){
    ...
    if ($this->form_validation->run() == TRUE) 
    {
      ...
      //print_r("$start_time > $row_c->jam_mulai && $start_time < $row_d->jam_akhir || $end_time > $row_c->jam_mulai && $end_time < $row_d->jam_akhir");exit;
      ...
      if( $start_time > $row_c->jam_mulai && $start_time < $row_d->jam_akhir || $end_time > $row_c->jam_mulai && $end_time < $row_d->jam_akhir){
          redirect(base_url() . 'admin/book_room/search_room/' . '1');
      ...
      //redirect(base_url() . 'admin/jadwal_booking/');'mailing/meetingMail/' . $id_booking
